...
import { IRechargeSearch, IRecharge } from '@/types/modules/recharge'
let searchForm = reactive<IRechargeSearch>({name: '', code: '', personTypeId: '', startTime: null, endTime: null})
let searchForms: any[] = [
  {el: 'input', prop: 'name', label: '用户', placeholder: '请输入用户'},
  {el: 'input', prop: 'code', label: '用户号', placeholder: '请输入用户号'},
  {el: 'select',  prop: 'personTypeId', label: '用户类型', placeholder: '请选择用户类型',
    options: [{label: '固定用户',value: '1'},{label: '不是',value: '2'},{label: '发生的',value: '3'}]},
  {el: 'timeRange',  propStart: 'startTime', propEnd:'endTime', label: '充值时间'},
  {el: 'button',options: [
    {name: '搜索',color: '#0081ff', icon:'search'},
    {name: '重置',color: '#6d7889', icon:'close'},
    {name: '普通充值',color: '#07b86c'},
    {name: '手动扣款',color: '#07b86c'}]},
]
let columns: any[] = [
  {label: '用户', prop: 'name'},
